...
import wanted.commerce.service.query.repository.ProductSearchRepository;
...
public class ProductSearchModelSyncer {
    ...
    public void consumeProductEvents (
        ...
        try {
            ...
            log.error("Error paring CDC event: {}", messageValue, e);
        } catch (Exception e) {
            log.error("Error paring CDC event: {}", messageValue, e);
        }
    }
    ...
    private void handleProductEvent(CdcEvent event) {
        ...
        // products 테이블 이벤트는 항상 전체 문서로 ㅓ리
    ...
    public void handleProductCategoryEvent(CdcEvent event) {
        Map<String, Object> data;
        Long productId;
        ...
        if (optionalDocument.isEmpty()) {
            log.warn("Product document not found category update: {}", productId);
            return;
        }
    ...
    public void handleProductPriceEvent(CdcEvent event) {
        Map<String, Object> data;
        Long productId;
    ...
    private void handleProductTagEvent(CdcEvent event) {
        ...
        // 태그 목록을 조회해야 함 - 기본 문서 필요
        Optional<ProductSearchDocument> optionalDocument = productSearchRepository.findById(productId);
        if (optionalDocument.isEmpty()) {
            log.warn("Product document not found tag update: {}", productId);
            return;
        }
