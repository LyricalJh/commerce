...
import wanted.commerce.query.document.*;
...
@Slf4j
public class OtherDocumentModelSync {
    ...
    public void consumeTagEvents(
        ...
        try {
            ...
            // 값 데이터 파싱
            CdcEvent event = objectMapper.readValue(messageKey, CdcEvent.class);
    ...
    private void handleTagEvent(CdcEvent event) {
        ...
        if (event.isDelete()) {
            ...
            log.info("Deleted brand tag with ID: {}", tagId);
        }
    ...
    private void handleBrandEvent(CdcEvent event) {
        ...
        BrandDocument brandDocument = BrandDocument.builder()
                .id(brandId)
                .name(getStringValue(data, "name"))
                .slug(getStringValue(data, "slug"))
                .description(getStringValue(data, "description"))
                .logoUrl(getStringValue(data, "logoUrl"))
                .website(getStringValue(data, "website"))
                .build();

        brandDocumentRepository.save(brandDocument);
        log.info("Saved brand document with ID: {}", brandDocument.getId());
    }
    ...
    private void handleSellerEvent(CdcEvent event) {
        ...
        SellerDocument document = SellerDocument.builder()
                ...
                .contactEmail(getStringValue(data, "contactEmail"))
                .contactPhone(getStringValue(data, "contactPhone"))
